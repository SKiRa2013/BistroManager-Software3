...
from .models import (
    ...
    Restaurante, Producto, Order,
)

# Historia 6: Ver el menú público
def public_menu(request, restaurant_slug):
    restaurante = get_object_or_404(Restaurante, slug=restaurant_slug)
    productos = Producto.objects.filter(restaurante=restaurante, is_available=True)
    return render(request, 'menu_publico.html', {'restaurante': restaurante, 'productos': productos})

# Historia 7: API para recibir la comanda
@csrf_exempt
@require_http_methods(["POST"])
def api_registrar_comanda(request):
    # Aquí procesas el JSON que viene del formulario de la comanda
    data = json.loads(request.body)
    # Lógica para crear el pedido en la base de datos...
    return JsonResponse({"ok": True, "mensaje": "Pedido registrado"})


require_http_methods(["GET"])
def pedidos_lista_api(request):
    # Filtramos pedidos pendientes
    pedidos = Order.objects.filter(estado='Pendiente')
    data = [
        {
            "producto_nombre": p.producto.nombre, # O el campo que tengas
            "cantidad": p.cantidad,
...
def productos_api(request):
    if request.method == "GET":
        productos = Producto.objects.all().values('id', 'nombre', 'precio', 'stock_quantity', 'is_available')
        return JsonResponse({"productos": list(productos)})
    
    if request.method == "POST":
        # Lógica para guardar un nuevo producto (ModelForm o creación directa)
        data = json.loads(request.body)
...
def configuracion_api(request):
    rc = _get_restaurant_config() # Función que ya tenías en tu código
    
    if request.method == "GET":
        return JsonResponse({"is_open": rc.is_open})
    
    if request.method == "POST":
        data = json.loads(request.body)
        rc.is_open = data.get("is_open", rc.is_open)
        rc.save()
        return JsonResponse({"is_open": rc.is_open})
    
def es_restaurante_abierto():
    rc = _get_restaurant_config() # Tu función existente
    return rc.is_open

# Uso en tus nuevas vistas (Historia 6 y 7)
def public_menu(request, restaurant_slug):
    if not es_restaurante_abierto():
        return render(request, 'restaurante_cerrado.html')
    
@require_http_methods(["POST"])
def api_registrar_comanda(request):
    if not es_restaurante_abierto():
        return JsonResponse({"error": "El restaurante está cerrado"}, status=403)
